...
Revisa los expedientes "en la Dirección" contra el portal de expedientes de la provincia y evita tener que anotar en un papel y después cargar a mano si alguno ya salió. La **búsqueda general** del portal (`https://apps2.entrerios.gov.ar/consulta/expedientes/`, por DNI/asunto/fecha) tiene un reCAPTCHA real y esa no se automatiza. Pero la **vista directa por número** (`#/ver?nro=...`, la misma que usa el botón 🔗 "Consultar en el portal" de la tabla) no pasa por esa búsqueda ni pide captcha, y la API que usa esa vista (`expedientes-api/detalle/{numero}?intranet=false`) tampoco: alcanza con conocer el número de expediente.

- **"🤖 Revisar automáticamente"**: el Apps Script consulta esa API por cada expediente en la Dirección (acción `ubicacion`, ver más abajo) y lee el `ente_destino` del movimiento más reciente. Si sigue conteniendo "asuntos jurídicos" no hace nada; si no, deja el expediente marcado con el destino y la fecha detectados, con la fecha de salida ya precargada — listo para "Guardar" (uno por uno o todos juntos con "✅ Aplicar los N cambios detectados"). Nunca escribe nada sin que alguien lo confirme. Los que no se pudieron consultar (portal caído, expediente no encontrado) quedan pendientes con un aviso, sin bloquear el resto.
- **"🔗 Abrir portal"** (por expediente): para revisar a mano un caso puntual, sin esperar a la revisión automática.
- **"✅ Sigue en Jurídicos"** / **"📤 Ya salió"**: las mismas acciones manuales de siempre, por si se prefiere no usar la revisión automática.

**Requiere agregar al `doGet` del Apps Script** el manejo de `hoja === 'ubicacion'` (mismo patrón que ya usa `dictamenes`/`normativas` para pegarse a un servicio externo), con una llamada a `UrlFetchApp.fetch` hacia la API pública del portal. Sin ese agregado, "Revisar automáticamente" simplemente no encuentra nada y todo queda para revisión manual con "🔗 Abrir portal" — no rompe nada.
...
- Cada agente puede tener una **contraseña propia** para ver su panel. Se la puede fijar/cambiar/quitar **el mismo agente** desde "Mi Panel" (botón "Fijar mi contraseña" / "Cambiar mi contraseña" una vez que ya está viendo su panel desbloqueado), o el **Director** desde la gestión de agentes (ver abajo, botón "Asignar/Cambiar clave" en cada fila). Se guarda como **hash SHA-256** en la columna `clave_hash` de la hoja `agentes` (nunca en texto plano).
- **El Director es `Alonso`** (constante `DIRECTOR` en el código). Se autentica con la **clave maestra** (`CLAVE_HASH`) y con eso accede a **todas las pestañas y a todos los paneles** de los agentes, sin necesidad de la contraseña individual de cada uno.
...
- **La pestaña "Agentes" ya no aparece en la barra de pestañas.** Solo el Director puede llegar a ella: en "Mi Panel" hay un botón "🔒 Acceso Director" que pide la clave maestra; al ingresarla correctamente se despliega directo la gestión de agentes (gráficos, carga de trabajo, tiempos de resolución, alta/edición de agentes y cumpleaños). Una vez desbloqueada la sesión, ese mismo botón cambia a "🔒 Gestión de agentes" para volver a entrar sin pedir la clave de nuevo; desde ahí hay un botón "← Volver a Mi Panel" para salir.
